...
| 014 | Sync-Mail-Versand kann beim ersten Submit timeoutten, wenn SMTP träge ist | Hard-Timeout 5 s für Sync-Versand. Bei Timeout (oder anderem Versagen): Fallthrough zum V1.0-Pfad – Status bleibt `new`, Owner-Freigabe-Schleife greift. UI zeigt „Anfrage eingegangen – wir melden uns" statt „bestätigt". Schlechter UX als Sync-Erfolg, aber keine stille Fehlbestätigung wenn die Mail nie ankommt. |
